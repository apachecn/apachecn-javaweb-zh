...
企业应用通常需要通过应用复杂的业务规则来处理批量信息。有些应用需要自动作业来运行，并提供大块数据作为进一步处理的输入。此类功能始终是基于时间的作业，不需要任何用户干预。批处理广泛应用于银行和保险领域，在这些领域中，大量数据在预定时间进行处理。**作业**是一个流程，**批处理作业**意味着一组在预定时间运行以执行任务的流程。

# Spring批简介

SpringBatch 本身就是一个批处理框架，用于开发应用来执行批处理作业。它支持批处理优化和作业划分，并且具有高度可扩展性，这促使我们在批处理应用开发中考虑它。

## 使用Spring批的用例

让我们列出几个可以在应用中使用 Spring batch 的用例：
...
![Architecture of a batch job](img/7320OS_04_01.jpg)
...
您还可以使用 restful API 将数据汇集到批处理应用中或从中取出。在下一节中，让我们使用 Spring 批处理框架来创建作业。我们首先来看依赖关系。

# 对Spring批次的依赖性

要开始使用 Spring 批处理，我们需要查看依赖项。假设用户熟悉 Maven 应用，我们可以查看需要添加到`pom.xml`文件以使用 Spring Batch 的以下依赖项：
...
## 使用 Tasklet 接口创建示例批处理应用

让我们创建另一个在命令行上运行的批处理应用。此批处理应用打印消息。我们在本章的开头已经谈到了 Tasklet。作业包括步骤和步骤，步骤可以是两种类型之一：块样式步骤和小任务。
...
![Creating a sample batch application using the Tasklet interface](img/7320OS_04_02.jpg)

让我们创建一个名为`Chapter4-SpringBatchCommandLine`的简单 Java 批处理应用项目
...
让我们创建另一个批处理应用，从目录中读取 CSV 文件，并使用`commandlinerunner`运行作业。输出同样是一个 CSV 文件，可在`output`文件夹中使用。
...
我们将从应用开发开始，并检查这些`ItemReader`实现类是如何使用的。

1.  使用 Maven 创建 SpringJava 应用，并将其命名为`SpringBatchCommandLine-Chapter4Example2`。
...
让我们看看`FlatFileItemReader`的原型，了解它在应用中的作用：
...
Spring 批处理框架提供了一个选项，用于在应用中配置外部调度程序。

让我们将 Quartz 调度器与 Spring 批处理应用集成。Quartz 是一个基于 Java 的开源调度器。我们将使此应用读取文件，但我们将集成 Quartz 调度程序来执行调度。

1.  创建一个名为`SpringBatchQuartzExample`的简单 Maven 应用。
2.  使用与上一个应用中相同的`pom.xml`文件。
...
在下一节中，我们将了解如何对 Spring 批处理应用进行一些单元测试。

# 单元测试Spring批量应用

让我们演示如何为 Spring Batch 应用编写测试用例：
...
在本章中，我们学习了如何创建基于 Spring 的批处理应用来读取 CSV 文件。我们还演示了如何使用 SpringBatch 读取 XML 文件。最高级的主题是对作业进行分区，并将作业运行到单独的线程中。我们还集成了 SpringBatch 和 Quartz 调度程序。
