...
*   将 JSF 与 SpringSecurity集成
*   JSF 与基于表单的 SpringSecurity
*   使用 Spring Security 显示登录用户的 JSF 和基于表单的身份验证
*   使用 JSF 和基于摘要/哈希的 SpringSecurity
*   使用 SpringSecurity使用 JSF 注销
*   使用 SpringSecurity和 JSF 对数据库进行身份验证
*   使用 JSF 和 SpringSecurity的 ApacheDS 身份验证
*   JSF 和 SpringSecurity的身份验证错误消息

# 导言

有许多应用程序是用 ApacheFaces/JSF 开发的。它不像 Struts 2 那样是一个面向操作的框架，而是纯粹用于视图层。为了用 JSF 实现 SpringSecurity，我们需要找到一些解决方法。让我们看一些关于 JSF 和 SpringSecurity的方法。
...
让我们在 Eclipse 中创建一个简单的 ApacheMyFaces 应用程序。另外，让我们将 SpringSecurity集成到 JSF 中，然后演示基本的身份验证。
...
# 具有基于表单的 SpringSecurity的 JSF

在这一节中，我们将使用 JSF 和 Spring Security 实现基于表单的身份验证。将 ApacheMyFaces 与 SpringSecurity集成并不像 Struts 2 集成那样简单。

这需要一个变通办法。ApacheMyfaces 无法理解`/j_spring_security`方法。解决方法是在托管 Bean 类中创建一个自定义登录方法。我们将使用 JSF 外部上下文类将身份验证请求传递给 SpringSecurity框架。
...
当用户访问 URL 时，他们将被重定向到登录页面。然后，用户输入其凭证并单击**提交**。`ExternalContext`对象是使用`PhaseListener`实现的`FacesContext`对象实例化的。`context`对象通过`'j_spring_security_check'`URL 传递给请求对象。SpringSecurity将执行身份验证和授权。身份验证失败时，将引发异常。
...
在这一节中，我们将使用 JSF 和 SpringSecurity实现摘要身份验证。使用其中一种加密算法对用户密码进行哈希运算，并在`.xml`文件中进行配置。配置文件中还提到了用于散列密码的算法。
...
执行以下步骤在 JSF 应用程序中实现 SpringSecurity注销：
...
`CredentialManager`类实现的`phaseListener interface`。`doSpringSecurityLogout`方法通过使用`ExternalContext`创建上下文对象来处理 Spring 注销。然后，上下文将注销请求提交给 SpringSecurity框架，该框架将用户注销。
...
数据源参考在`Sping-security.xml`文件中给出。当用户点击**登录**时，SpringSecurity过滤器会调用数据库认证相关类，读取`db-beans.xml`文件建立连接。`<jdbc-user-service>`标记通过执行查询并根据用户在浏览器中提交的参数从数据库中检索用户信息来实现数据库身份验证。
...
JSF 过滤器用于委托。Spring 过滤器用于身份验证。我们使用 ldap 身份验证提供程序为 SpringSecurity引擎设置 ldap 参数。当应用程序收到身份验证和授权请求时，spring security ldap provider 设置 ldap 参数，并使用 ldap 服务器 url 参数连接 ldap。然后，它检索用户详细信息，并将其提供给 Spring 身份验证管理器和过滤器，以处理身份验证的响应。
...
*   [第四章](04.html "Chapter 4. Spring Security with Grails")*圣杯SpringSecurity*