...
# 踩过 WebSocket 并在Spring 4 中选择后退选项
...
假设您被要求开发一个基于浏览器的聊天室应用，其中匿名用户可以加入聊天室，并且用户发送的任何文本都应该发送给所有活动用户。这意味着我们需要一个所有用户都应该订阅的主题，任何用户发送的消息都应该广播给所有用户。弹簧 WebSocket 功能满足这些要求。在Spring，使用 STOMP over WebSocket，用户可以以类似于 JMS 的方式交换消息。在本节中，我们将开发一个聊天室应用，并解释 SpringWebSocket 的一些特性。
