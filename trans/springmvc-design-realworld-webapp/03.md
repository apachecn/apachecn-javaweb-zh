...
Spring 数据组合包含用于官方 Spring 数据项目未涵盖的更多数据存储的社区模块。几个非常流行的开源和专有数据库的社区正在为这些项目做出贡献，这使得 Spring Data 成为构建企业应用程序数据访问层的经验证解决方案的优秀来源，而不管底层数据存储是什么。Cassandra、Neo4J、Couchbase 和 ElasticSearch 是基于 Spring 数据的社区项目的一些例子。
