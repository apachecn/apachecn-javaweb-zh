...
为了管理我们在本章前面介绍的 API 对象，`kubectl apply`命令是您需要了解的唯一命令。它是一个声明性命令；也就是说，作为一个操作符，我们要求 Kubernetes 将我们给出的对象定义应用于命令。然后由Kubernetes决定到底需要做什么。
