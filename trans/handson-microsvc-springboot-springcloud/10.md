...
从前面的源代码中，我们可以看到，`WebClient.builder`用`@LoadBalanced`注释，这使得它知道在发现服务器 Netflix Eureka 中注册的微服务实例。详见[第 9 章](09.html)中*与 Netflix Eureka 在SpringCloud中的服务发现*一节*使用 Netflix Eureka 和 Ribbon 添加服务发现*。
