# 使用 Netflix Eureka 和 Ribbon 添加服务发现
...
服务发现的概念在[第一章](01.html)、*微服务简介*中进行了描述；有关更多信息，请参阅*服务发现*部分。Netflix Eureka 在[第 8 章](08.html)、*春云简介*中作为发现服务引入；有关更多信息，请参阅*Netflix Eureka 作为发现服务*部分。在我们开始讨论实施细节之前，我们将了解以下主题：
...
![](Images/70f0682d-4f5b-4fae-a1a0-d96ea3c3c254.png)
...
![](Images/a6dc8c02-132d-482d-bf64-2b2994b29eca.png)
 ...
 **![](Images/39bdb1ab-dd3a-4238-8fe8-9b112d5f2125.png)
...
当弹性 4J 与弹簧靴一起使用时，我们将能够使用其弹簧靴执行器`health`端点监测微服务中断路器的状态。例如，我们可以使用`curl`来查看断路器的状态，即`myService`：
...
![](Images/3fb19393-c8f6-46e4-a14b-f0850f75131e.png)

如果出现故障且电路**开路**，则会做出如下响应：

![](Images/0256fdb6-55bb-4ad8-9e24-532f801c9916.png)
...
![](Images/0b718675-03d1-4367-8114-0def71168052.png)
...
![](Images/6b7b0b09-bac1-4528-ad88-ab30d6410fde.png)
...
![](Images/3e46d604-73a8-4747-8af4-23865c058d65.png)
...
SpringCloud 附带了如何与 Netflix Eureka 等发现服务进行通信的抽象，并提供了一个名为`DiscoveryClient`的接口。这可用于与发现服务交互，以获取有关可用服务和实例的信息。`DiscoveryClient`接口的实现还能够自动向 discovery server 注册 Spring 引导应用程序。
...
![](Images/841fe8fc-968b-40e7-85fa-60fa73153679.png)
...
**   将依赖项添加到`spring-cloud-starter-netflix-eureka-server`。*   将`@EnableEurekaServer`注释添加到应用程序类中。*   添加一个 Dockerfile，类似于用于我们微服务的 Dockerfile，但我们导出的是 Eureka 默认端口`8761`，而不是我们微服务的默认端口`8080`。*   将 Eureka 服务器添加到我们的三个 Docker Compose 文件中，即`docker-compose.yml`、`docker-compose-partitions.yml`和`docker-compose-kafka.yml`：*
...
1.  如前所述，在应用程序类中添加负载平衡器感知的`WebClient`构建器，即`se.magnus.microservices.composite.product.ProductCompositeServiceApplication`：
...
![](Images/b53f6c5c-f964-4731-8907-8482ddd11cca.png)
...
![](Images/249647b6-1f89-4d9a-800c-d84d740893f6.png)
...
![](Images/66ef4c5e-20f9-4056-8bdb-e4398a194d4e.png)
...
![](Images/ccd79bc6-a3e1-43aa-891f-553c6f727741.png)
...
![](Images/cadd482e-aa89-4949-972e-d8fb4f3f1145.png)
...
![](Images/25abdb94-b95c-4cb9-baea-c5917f2ce8af.png)
...
![](Images/91291ded-566c-4cea-b151-aeea0e4c6390.png)
...
![](Images/a783eca8-e0b7-48be-947d-c972032e4c9b.png)
...
![](Images/4c8c141a-2404-4b8c-bf33-b7a0b59c5476.png)

这是第二个答复：

![](Images/03c25c97-63a7-49d6-82b1-72ea5a69ac79.png)
...
1.  要将使用 Spring Initializer 创建的 Spring 引导应用程序转变为成熟的 Netflix Eureka 服务器，需要什么？
