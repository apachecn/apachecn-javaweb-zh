...
服务发现的概念在[第一章](01.html)、*微服务简介*中进行了描述；有关更多信息，请参阅*服务发现*部分。Netflix Eureka 在[第 8 章](08.html)、*SpringCloud简介*中作为发现服务引入；有关更多信息，请参阅*Netflix Eureka 作为发现服务*部分。在我们开始讨论实施细节之前，我们将了解以下主题：
...
当Resilience4j 与SpringBoot一起使用时，我们将能够使用其SpringBoot执行器`health`端点监测微服务中断路器的状态。例如，我们可以使用`curl`来查看断路器的状态，即`myService`：
