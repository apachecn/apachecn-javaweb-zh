...
本章中我们有很多内容要介绍，但在深入了解如何利用 SpringSecurity来实现 OAuth 2 和 JWT 之前，我们首先要创建一个日历应用的基线，该应用没有 Thymeleaf 或任何其他基于浏览器的用户界面。
...
`@EnableResourceServer`注释表示包含应用启用 SpringSecurity过滤器的意图，该过滤器通过传入的 OAuth2 令牌对请求进行认证：
