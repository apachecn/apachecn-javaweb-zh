...
*   配置 SpringSecurity以使用客户端证书对用户进行认证
...
我们将在配置 SpringSecurity以访问通过证书认证的用户的信息时了解这一点的重要性。在 Tomcat 中设置证书认证之前，我们还有最后一步，这将在下一节中解释。
...
但是，您会注意到，如果您试图访问站点的受保护部分，例如“我的事件”部分，您将被重定向到登录页面。这是因为我们还没有配置 SpringSecurity来识别证书中的信息。此时，客户端和服务器之间的协商已在 Tomcat 服务器本身停止。
...
安全登录过程的一个重要组成部分仍然缺失，即已验证用户的授权。这就是我们对 SpringSecurity的配置，我们必须向 SpringSecurity添加一个组件，该组件将识别来自用户 HTTP 会话（由 Tomcat 填充）的证书认证信息，然后根据 SpringSecurity`UserDetailsService`调用验证提供的凭证。调用`UserDetailsService`将确定 Spring Security 是否知道证书中声明的用户，然后根据通常的登录规则分配`GrantedAuthority`。
...
注意`.x509()`方法引用了我们现有的`userDetailsService()`配置。为简单起见，我们使用[第 5 章](05.html)中所述的`UserDetailsServiceImpl`实现，*使用 SpringData进行认证*。但是，我们可以轻松地将其与任何其他实现（即，[第 4 章](04.html)、*基于 JDBC 的认证*中介绍的基于 LDAP 或 JDBC 的实现）交换。
...
# SpringSecurity证书认证的工作原理

让我们在下图的帮助下，回顾一下参与审查和评估客户机证书并将其转换为 SpringSecurity认证会话的各种参与者：
...
相反，在典型的客户端证书认证环境中，根本不支持其他认证方法（请记住，Tomcat 希望在 SpringSecurity表单登录之前就获得证书）。因此，保留重定向到表单登录页面的默认行为是没有意义的。相反，我们将修改入口点，使用`o.s.s.web.authentication.Http403ForbiddenEntryPoint`返回`HTTP 403 Forbidden`消息。继续并在您的`SecurityConfig.java`文件中进行以下更新，如下所示：
...
某些环境也可能同时支持基于证书和基于表单的认证。如果在您的环境中是这种情况，那么使用 SpringSecurity支持它也是可能的（而且很简单）。我们可以简单地保留默认的`AuthenticationEntryPoint`界面（重定向到基于表单的登录页面），并允许用户在不提供客户端证书的情况下使用标准登录表单登录。
...
在本章中，我们研究了基于客户端证书的认证的体系结构、流程和 SpringSecurity支持。我们已经介绍了客户端证书（相互）认证的概念和总体流程。我们探讨了为自签名 SSL 和客户端证书场景配置 ApacheTomcat 所需的重要步骤。

我们还了解了如何配置 SpringSecurity以理解客户端提供的基于证书的凭据。我们介绍了与证书认证相关的 SpringSecurity类的体系结构。我们还知道如何配置 Springbean 风格的客户端证书环境。我们还讨论了这种认证的优缺点。
