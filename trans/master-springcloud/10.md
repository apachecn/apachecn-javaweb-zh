...
对该地区另外两种流行产品的支持仍处于开发阶段。以下项目尚未添加到官方的 Spring 云发布系列中：
...
![](assets/8fa25939-b447-4d22-af84-19dab28e9745.png)
...
![](assets/208a2cee-71ec-4469-b222-39572887b4ea.png)
...
![](assets/a1a34b42-27d3-46d1-814b-fc20ffd77a3e.png)
...
![](assets/62ad9459-eb41-4770-8035-f2a23dee1e42.png)

领事服务器代理在`8500`端口公开，成员代理在`8501`和`8502`端口公开。即使 microservice 实例将自己注册到成员代理，集群的所有成员都可以看到它：

![](assets/0003cbac-7ff3-4286-ac65-3c13cf98a2ad.png)
...
![](assets/f0b420c2-a7db-4403-9a36-107311c5d2ac.png)

每个密钥都可以更新或删除：

![](assets/85ffa69a-6da9-49aa-b8fc-ef419d3a28ce.png)
...
![](assets/bffd5327-fdb1-4ea1-9e4f-8dd99169c30a.png)
...
客户端的用法与其他与服务发现相关的 Spring 云项目相同。应用程序主类或`@Configuration`类应该用`@EnableDiscoveryClient`注释。默认服务名称、实例 ID 和端口分别取自`spring.application.name`、Spring 上下文 ID 和`server.port`。
...
![](assets/fdbd83d4-a14c-43f6-8302-5a761da38a9d.png)
...
![](assets/7494c667-24b5-4a33-aef6-fbb27e519e61.png)

# 总结

在本章中，我将引导您了解两个 SpringCloud 项目 Concur 和 Zookeeper 的主要功能。我不仅关注 Spring 云的功能，还向您介绍了如何启动、配置和维护其工具实例。我们讨论了更高级的场景，例如使用 Docker 设置由多个成员组成的集群。在那里，您有机会看到 Docker 作为开发工具的真正威力。它允许我们只使用三个简单的命令初始化一个由三个成员组成的集群，而不需要任何额外的配置。

在使用 SpringCloud 时，Concur 似乎是 Eureka 作为发现服务器的重要替代品。我不能对动物园管理员说同样的话。正如你们可能已经注意到的，我写的关于领事的文章比关于动物园管理员的要多得多。此外，春云将动物园管理员视为第二选择。与 SpringCloudConcur 相比，它仍然没有分区机制，也没有监视已实现的配置更改的功能。你不应该对此感到惊讶。Concur 是一种现代解决方案，旨在满足最新体系结构（如基于微服务的系统）的需求，而 Zookeeper 是一种密钥/价值存储，用作分布式环境中运行的应用程序的服务发现工具。但是，如果在系统中使用 Apache 基础堆栈，则值得考虑该工具。有鉴于此，您可以利用 Zookeeper 和其他 Apache 组件（如 Camel 或 Karaf）之间的集成，轻松发现使用 Spring 云框架创建的服务。
