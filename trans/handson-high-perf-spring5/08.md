...
在多线程应用程序中，可能存在多线程试图访问共享资源并产生错误和意外结果的情况。我们需要确保资源一次只被一个线程使用，这可以通过同步实现。`synchronized`关键字用于实现同步；在 Java 中定义任何同步块时，只有一个线程可以访问该块，其他线程被阻塞，直到块中的线程退出该块。
...
![](assets/bc28c2a8-acac-4718-a085-9dab2fe53a72.png)
