...
现在，在开发了这些微服务之后，我们很有兴趣了解一下**在线餐桌预订系统**（**OTRS**）提供的服务如何被 web 或移动应用使用。我们将使用 AngularJS/Bootstrap 开发 web 应用（UI），以构建 web 应用的原型。此示例应用将显示此示例项目（一个小型公用事业项目）的数据和流程。此 web 应用也是一个示例项目，将独立运行。早些时候，web 应用是在包含 UI 和服务器端代码的单一 web 存档（扩展名为`.war`的文件）中开发的。这样做的原因很简单，因为 UI 也是使用 Java 与 JSP、servlet、JSF 等一起开发的。如今，UI 正在使用 JavaScript 独立开发。因此，这些 UI 应用也作为单个微服务部署。在本章中，我们将探讨如何开发这些独立的 UI 应用。我们将开发和实现 OTRS 示例应用，无需登录和授权流程。我们将部署一个非常有限的功能实现，并涵盖高级 AngularJS 概念。关于 AngularJS 的更多信息，您可以参考*AngularJS 示例*、*Chandermani*、*Packt Publishing*。
...
现在，既然我们已经准备好 HTML5Web 应用设置，我们就可以学习 AngularJS 的基础知识了。这将帮助我们理解 AngularJS 代码。本节描述了您可以通过使用 AngularJS 文档或参考其他 Packt 发布资源来理解示例应用和进一步探索的高级理解。
...
*   **模型**：模型是包含应用数据的 JavaScript 对象。它们还表示应用的状态。
...
模块是我们为任何 AngularJS 应用定义的第一件事。模块是包含应用不同部分的容器，如控制器、服务、过滤器等。AngularJS 应用可以在单个模块或多个模块中编写。AngularJS 模块还可以包含其他模块。

许多其他 JavaScript 框架使用`main`方法来实例化和连接应用的不同部分。AngularJS 没有`main`方法。由于以下原因，它将模块用作入口点：

*   **模块化**：您可以按功能划分和创建应用，也可以使用可重用组件。
*   **简单性**：您可能会遇到复杂而庞大的应用代码，这使得维护和增强成为一个难题。没有更多：AngularJS 使代码简单易懂。
*   **测试**：它使单元测试和端到端测试更容易，因为您可以覆盖配置，只加载所需的模块。

每个 AngularJS 应用都需要一个模块来引导 AngularJS 应用。引导我们的应用需要以下三个部分：
...
*   **应用 DOM 配置**：这告诉 DOM 元素的 AngularJS 位置，应该在该位置进行引导。可以通过以下两种方式之一进行：
...
AngularJS 大量使用依赖注入模式，使用注入服务（`$injector`来实例化和连接我们在 AngularJS 应用中使用的大多数对象。此注入器创建两种类型的对象服务和专用对象。
...
现在我们对服务有了一些了解，可以说服务有两种常见用途：组织代码和跨应用共享代码。服务是单例对象，由 AngularJS 服务工厂延迟实例化。我们已经看到了一些内置的 AngularJS 服务，比如`$injector`、`$log`等等。AngularJS 服务的前缀为`$`符号。

# 范围

在 AngularJS 应用中，广泛使用两种类型的示波器-`$rootScope`和`$scope`：
...
AngularUI 团队构建了 UI 路由器，一个 AngularJS 路由框架。UI 路由器不是核心 AngularJS 的一部分。UI 路由器不仅更改路由 URL，而且在用户单击 SPA 中的任何链接时还更改应用的状态。因为 UI 路由器也可以进行状态更改，所以您可以在不更改 URL 的情况下更改页面的视图。这是可能的，因为 UI 路由器管理应用状态。

如果我们将 SPA 视为状态机，那么状态是应用的当前状态。创建路由链接时，我们将在 HTML 链接标记中使用`ui-sref`属性。链接中的`href`属性将由此生成，并指向在`app.js`中创建的应用的某些状态。
...
如你所知，我们正在开发水疗中心。因此，一旦应用加载，您就可以在不刷新页面的情况下执行所有操作。与服务器的所有交互都使用 AJAX 调用执行。现在，我们将使用第一节中介绍的 AngularJS 概念。我们将介绍以下场景：
...
主页是任何 web 应用的主页。为了设计主页，我们将使用 Angular UI 引导，而不是实际的引导。Angular UI 是引导的 Angular 版本。主页将分为三个部分：

*   标题部分将在右上角包含应用名称、搜索表单和用户名。
...
然后，在添加几个`meta`标记和应用的标题之后，我们还将定义重要的`meta`标记`viewport`。`viewport`用于响应用户界面设计。
...
在接下来的几行中，我们将定义应用的样式表。我们从 HTML5 样板代码中添加了`normalize.css`和`main.css`。我们还添加了应用的客户 CSS`app.css`。最后，我们添加了 bootstrap3css。除了客户`app.css`之外，它还引用了其他 CSS。这些 CSS 文件中没有任何更改：
...
在`body`标记中，我们使用
`ng-controller`属性定义应用的控制器。页面加载时，它会将应用的名称告知控制器，如下所示：

```java
<body ng-controller="otrsAppCtrl"> 
```

然后，我们定义主页的`header`部分。在`header`部分，我们将定义应用标题`Online Table Reservation System`。此外，我们还将定义搜索餐厅的搜索表单：
...
我们将使用 REST 服务器（包括边缘服务器）访问所有内容。包括 UI 在内的外部应用将使用边缘服务器主机访问应用。您可以在全局常量文件中配置它，然后在需要时使用它。这将允许您在单个位置配置 REST 主机，并在其他位置使用它：
...
`restaurants.js`表示我们将用于餐厅的应用的角度服务。我们知道，服务有两种常见用途：组织代码和跨应用共享代码。因此，我们创建了一个餐馆服务，它将在不同的模块中使用，如搜索、列表、详细信息等。
...
由于我们计划在 UI 应用开发中使用最新的技术堆栈，我们将使用 Node.js 和**npm**（**Node.js package manager**）为开发服务器端 JavaScript web 应用提供开源运行时环境。
...
3.  导航到文件|新项目。此时会出现一个新的项目对话框。在类别列表下选择 HTML5/JavaScript，在项目选项下选择 HTML5/JS 应用，如以下屏幕截图所示：
...
7.  现在，一旦单击 Finish，就可以看到 HTML5/JS 应用目录和文件。目录结构将如以下屏幕截图所示：
...
*   **Bower**：需要管理 OTRS 应用的 JavaScript 库
*   **吞食**：一个任务执行者，需要像蚂蚁一样构建我们的项目
*   **节点**：用于执行我们的服务器端 OTRS 应用

Bower 是一个依赖项管理工具，与 npm 类似。Npm 用于安装 Node.js 模块，而 Bower 用于管理 web 应用的库/组件。
...
16.  接下来，我们将修改`.bowerrc`文件，如下面的代码所示，以指定 Bower 将存储`bower.json`中定义的组件的目录。我们将在应用目录下存储 Bower 组件：
...
*   根据定义的`default_sequence`任务，首先连接到服务器，然后构建 web 应用，并监视更改。手表将有助于呈现我们在代码中所做的更改，并将立即显示在 UI 上。
...
在本章中，我们了解了新的动态 web 应用开发。
这些年来，它已经完全改变了。web 应用前端完全是用纯 HTML 和 JavaScript 开发的，而不是使用任何服务器端技术，如 JSP、servlet、ASP 等。使用 JavaScript 开发 UI 应用现在有自己的开发环境，如 npm、Bower 等。我们已经探索了 AngularJS 框架来开发我们的 web 应用。它通过提供内置特性和对引导和处理 AJAX 调用的`$http`服务的支持，使事情变得更简单。

我希望您已经掌握了 UI 开发概述以及现代应用开发和与服务器端微服务集成的方式。在下一章中，我们将学习微服务设计的最佳实践和常见原则。本章将使用行业实践和示例提供有关微服务开发的详细信息。它还将包含 microservices 实现出错的地方以及如何避免此类问题的示例。